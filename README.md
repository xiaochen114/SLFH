...
> 📅 2026-08-04 中央大脑大规模改造日志见 [文档/中央大脑改造日志_20260804.md](文档/中央大脑改造日志_20260804.md)
...
- **系统分配 ID**：机器人注册时由中央大脑生成 `品牌-时间戳`（如 `绝影-20260804-143025`），不信任机器人自报名，保证全局唯一
- **品牌映射**：dog→绝影，drone→大疆，other→设备
- **指令标签路由**：每条指令带 `robot_id`（收件人标签），按标签下发到指定机器人；LLM 决策未指定时由规则引擎按能力/状态分配
- **告警独立存储**：告警写入独立 `alerts` 表，重启不丢
...
- 决策在**后台线程**异步执行，不阻塞调度循环（调度循环 2s 一跳始终响应）
- 优先调用大模型 API，不可用 → 降级规则模式，后台每 30s 重连
- 配置在 `配置.yaml` 的 `llm_api_url/key/model`

### 高危三件套（不依赖 LLM）

检测到火情等高危事件（priority≥2）时，立即执行：
1. **急停所有机器狗** — 硬逻辑，不等待 LLM
2. **报火警** — SSE 广播 `fire_alarm`，前端全屏红色声光报警
3. **记录事件** — 数据库留档，可追溯
